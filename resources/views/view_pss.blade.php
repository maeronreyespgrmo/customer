...
<div class="card-tools"> --}}
                        ...
                        <v-dialog v-model="dialog1" max-width="900">
                            <v-card>
                                ...
                                <v-card-text>
                                    <v-col cols="12">
                                        {{-- <input type="year" />
                                        <v-select v-model="dates" :items="dates_items" label="Select Date"></v-select> --}}
                                        <v-menu ref="menu" v-model="menu" :close-on-content-click="false"
                                            :return-value.sync="dates" transition="scale-transition" offset-y
                                            min-width="auto">
                                            <template v-slot:activator="{ on, attrs }">
                                                <v-combobox v-model="dates" label="Select Date" append-icon="mdi-calendar"
                                                    readonly v-bind="attrs" v-on="on" required></v-combobox>
                                            </template>
                                            <v-date-picker v-model="dates" no-title type="month" scrollable required
                                                required>
                                                <v-spacer></v-spacer>
                                    ...
                                    <v-col cols="12">
                                        <v-select v-model="hospital_name" :items="hospital_items" label="Select Hospital">
                                        </v-select>
                                    </v-col>
                                </v-card-text>
                                ...
                                <v-card-actions>
                                    ...
                                    <v-btn color="green darken-1" text @click="export_btn()">
                                        Ok
                                    </v-btn>
                                </v-card-actions>
                            </v-card>
                        ...
                        <v-data-table :headers="headers" :items="data" sort-by="id" class="elevation-1"
                            ...
                            <template v-slot:top>
                                <v-toolbar flat :style="{ padding: '15px' }">
                                    ...
                                    <v-col cols="4">
                                        <v-select v-model="selectedHospital" :items="selectedItemHospital"
                            ...
                            <template v-slot:item.actions="{ item }">
                                ...
                                <!-- <v-btn class="primary" @click="editItem(item)">View</v-btn> -->
                                <v-btn small class="success" @click="viewItem(item)">
                                    <v-icon small class="mr-2">mdi-account-search</v-icon>
                                    View
                                </v-btn>
                                <v-btn small class="primary" @click="editItem(item)">
                        ...
                        <v-row class="text-center px-4 align-center" wrap>
                            ...
                            {{-- <v-col cols="12" md="6">
                                <v-pagination v-if="data.length > 0" v-model="page" @input="next"
                                    :length="pageCount">
                                </v-pagination>
                            </v-col> --}}
                        </v-row>
...
@section('page_script')
    <script type="text/javascript" src="/js/view_pss_script.js?v=1"></script>
@endsection
